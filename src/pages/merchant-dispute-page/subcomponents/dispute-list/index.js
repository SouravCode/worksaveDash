...
function DisputesList({
  ...
  const closeTab = () => {
    ...
    getAccessToken().then(token => {
      ...
      getDispute(merchantDetails.id, token, "");
    });
  ...
  let totalDispute =
    ...
    (disputeStat && disputeStat.closedDisputes
      ...
      : 0);
  let totalDisputeTransactions = disputes ? disputes.length : 0;
  let totalSales =
    (disputeStat &&
  ...
  const onSuccess = () => {
    ...
    getAccessToken().then(token => {
      ...
      getDispute(merchantDetails.id, token, query);
    });
  ...
  const renderGridCell = data => {
    return (
      <tbody>
        <tr>
          <td>
            <p className="disputeAlign">
              <span>{data.data.transactionDate}</span>
            </p>
          </td>
          <td>
            <p className="disputeAlign">{data.data.transactionId}</p>
          </td>
...
const mapStateToProps = state => ({
  ...
  isLoaded: MerchantDashboard.selectors.isDisputeStatLoading(state)
});
